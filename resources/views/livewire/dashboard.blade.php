<div class="py-12 bg-gray-50 min-h-screen">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <!-- Active Projects -->
            <div
                class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100 p-6 flex items-center justify-between">
                <div>
                    <p class="text-sm font-medium text-gray-500 uppercase tracking-wider">進行中案件数</p>
                    <p class="mt-1 text-3xl font-bold text-gray-900">{{ $activeProjectsCount }}<span
                            class="text-sm font-normal text-gray-400 ml-1">件</span></p>
                </div>
            ...
            <div
                ...
                <div>
                    ...
                    <p
                        class="mt-1 text-3xl font-bold {{ $monthlyUnbilledTotal > 0 ? 'text-red-500' : 'text-gray-900' }}">
                        ¥{{ number_format($monthlyUnbilledTotal) }}</p>
                </div>
                <div
                    class="p-3 {{ $monthlyUnbilledTotal > 0 ? 'bg-red-50 text-red-500' : 'bg-gray-50 text-gray-400' }} rounded-full">
                    <svg class="w-8 h-8" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-100">
                <div class="p-6 border-b border-gray-100 flex justify-between items-center">
                    ...
                    <a href="{{ route('projects.index') }}" class="text-sm text-blue-600 hover:text-blue-800">全て見る
                        &rarr;</a>
                </div>
                    ...
                    @forelse($upcomingBillings as $billing)
                        <div class="p-4 hover:bg-gray-50 transition-colors flex justify-between items-center">
                            <div>
                                ...
                                <p class="text-xs text-gray-500">{{ $billing->project->customer->name }} -
                                    第{{ $billing->billing_round }}回</p>
                            </div>
                            <div class="text-right">
                                <p class="text-sm font-bold text-gray-800">¥{{ number_format($billing->amount_this_time) }}
                                </p>
                                <p
                                    class="text-xs {{ $billing->billing_date->isPast() ? 'text-red-500 font-bold' : 'text-gray-500' }}">
                                    {{ $billing->billing_date->format('Y/m/d') }}
                                </p>
                            </div>
                            <div class="ml-4">
                                <a href="{{ route('billings.edit', [$billing->project, $billing]) }}"
                                    class="text-blue-600 hover:bg-blue-50 p-2 rounded-lg text-sm">請求処理</a>
                            </div>
                        </div>
                    @empty
