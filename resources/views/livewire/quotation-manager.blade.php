<div class="p-6 bg-gray-50 min-h-screen font-sans">
    ...
    <!-- Header Summary -->
    <div
        class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 mb-6 sticky top-0 z-10 bg-opacity-95 backdrop-blur-sm">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-8 text-right">
                ...
                <div>
                    ...
                    <span
                        class="text-3xl font-extrabold text-blue-600 tracking-tight">¥{{ number_format($grandTotal) }}</span>
                </div>
            </div>
        </div>
    ...
    <div class="space-y-6 pb-20">
                    ...
                    <div class="flex items-center flex-1 space-x-4">
                        <button class="text-gray-400 hover:text-gray-600 transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg"
                                class="h-5 w-5 transform transition-transform {{ $item['expanded'] ? 'rotate-180' : '' }}"
                        ...
                        <div class="flex items-center space-x-2 w-1/2" wire:click.stop>
                            ...
                            <input type="text" wire:model.blur="items.{{ $index }}.name"
                ...
                @if($item['expanded'])
                    <div class="p-4 bg-white animate-fade-in">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    @foreach ($item['details'] as $detailIndex => $detail)
                                        <tr class="group hover:bg-blue-50/30 transition-colors">
                                            <td class="p-2">
                                                <input type="text"
                                                    class="w-full bg-gray-50 border-none rounded focus:ring-1 focus:ring-blue-500 text-gray-700 placeholder-gray-300 text-sm py-1.5 px-2"
                                                    wire:model.blur="items.{{ $index }}.details.{{ $detailIndex }}.name"
                                                    placeholder="詳細名称">
                                            </td>
                                            <td class="p-2">
                                                <input type="text"
                                                    class="w-full bg-gray-50 border-none rounded focus:ring-1 focus:ring-blue-500 text-gray-600 placeholder-gray-300 text-sm py-1.5 px-2"
                                                    wire:model.blur="items.{{ $index }}.details.{{ $detailIndex }}.specification"
                                                    placeholder="規格">
                                            </td>
                                            <td class="p-2">
                                                <input type="number" step="0.01"
                                                    class="w-full bg-white border border-gray-200 rounded text-right focus:ring-1 focus:ring-blue-500 text-gray-700 font-mono py-1.5 px-2"
                                                    wire:model.live="items.{{ $index }}.details.{{ $detailIndex }}.quantity">
                                            </td>
                                            <td class="p-2">
                                                <input type="text"
                                                    class="w-full bg-white border border-gray-200 rounded text-center focus:ring-1 focus:ring-blue-500 text-gray-600 py-1.5 px-2"
                                            ...
                                            <td class="p-2">
                                                <input type="number"
                                                    class="w-full bg-white border border-gray-200 rounded text-right focus:ring-1 focus:ring-blue-500 text-gray-500 font-mono py-1.5 px-2"
                                                    wire:model.live="items.{{ $index }}.details.{{ $detailIndex }}.cost_price">
                                            </td>
                                            <td class="p-2 text-right">
                                                <span
                                                    class="font-bold text-gray-800">¥{{ number_format($detail['total_price']) }}</span>
                                            </td>
                                            <td class="p-2 text-center">
                                                <button wire:click="removeDetail({{ $index }}, {{ $detailIndex }})"
                                                    class="text-gray-200 hover:text-red-400 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <div class="mt-4 pt-2 border-t border-dashed border-gray-200">
                            <button wire:click="addDetail({{ $index }})"
        ...
        <button wire:click="addItem"
            class="w-full py-4 border-2 border-dashed border-gray-300 rounded-xl text-gray-400 font-bold hover:border-blue-400 hover:text-blue-500 hover:bg-blue-50 transition-all">
            + 大項目を追加
        </button>
    </div>
</div>