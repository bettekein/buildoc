                    ...
                    @forelse ($customers as $customer)
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                                @if(!$showTrashed)
                                    <button wire:click="edit({{ $customer->id }})" class="text-indigo-600 hover:text-indigo-900 bg-indigo-50 hover:bg-indigo-100 px-3 py-1 rounded-md transition-colors">編集</button>
                                @endif
                                @if($showTrashed)
                                ...
                                @else
                                    <button wire:click="delete({{ $customer->id }})" class="text-red-600 hover:text-red-900 bg-red-50 hover:bg-red-100 px-3 py-1 rounded-md transition-colors" onclick="confirm('ゴミ箱に移動しますか？') || event.stopImmediatePropagation()">削除</button>
                                @endif
                            </td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-10 text-center text-gray-500">顧客が登録されていません。</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
