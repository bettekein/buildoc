<div class="p-6 bg-gray-50 min-h-screen font-sans">
    <div class="max-w-5xl mx-auto">
        @if($billing->exists)
            {{ Breadcrumbs::render('billings.edit', $billing) }}
        @else
            {{ Breadcrumbs::render('billings.create', $project) }}
        @endif

        <div class="flex justify-between items-center mb-6">
            <div>
                <h1 class="text-2xl font-bold text-gray-800">出来高請求書作成 (第{{ $billing->billing_round }}回)</h1>
                <p class="text-sm text-gray-500 mt-1">{{ $project->name }} （契約金額:
                    ¥{{ number_format($contract_amount) }}）</p>
            </div>
            <div>
                <a href="{{ route('projects.edit', $project) }}"
                    class="text-gray-500 hover:text-gray-700 underline text-sm">案件詳細に戻る</a>
            </div>
        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" wire:model="billing_number"
                                class="w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                            @error('billing_number') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">請求日 <span
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid grid-cols-2 gap-6 items-center bg-blue-50 p-4 rounded-lg">
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" step="0.01" wire:model.live.debounce.500ms="progress_rate"
                                        class="block w-full rounded-md border-blue-300 pl-3 pr-8 focus:border-blue-500 focus:ring-blue-500 text-right font-bold text-lg">
                                    <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center pr-3">
                                        <span class="text-gray-500 sm:text-sm">%</span>
                                    </div>
                                </div>
                                <p class="text-xs text-gray-500 mt-1">前回までの累計:
                                    {{ number_format($previous_billed_amount) }}
                                </p>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" wire:model.live.debounce.500ms="amount_this_time"
                                        class="block w-full rounded-md border-blue-300 pl-7 pr-3 focus:border-blue-500 focus:ring-blue-500 text-right font-bold text-lg">
                                </div>
                                @error('amount_this_time') <span class="text-red-500 text-xs">{{ $message }}</span>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-2 gap-6">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" wire:model.live.debounce.500ms="retention_release_amount"
                                        class="block w-full rounded-md border-gray-300 pl-7 pr-3 text-right">
                                </div>
                            </div>
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" wire:model.live.debounce.500ms="offset_amount"
                                        class="block w-full rounded-md border-gray-300 pl-7 pr-3 text-right">
                                </div>
                            </div>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 sticky top-6">
                    ...
                    <div class="space-y-3 text-sm">
                        ...
                        <div class="flex justify-between pt-4 items-center">
                            ...
                            <span
                                class="font-extrabold text-2xl text-blue-600">¥{{ number_format($final_billing_amount) }}</span>
                        </div>
                        ...
                        <div class="mt-8 space-y-3">
                            <button wire:click="save"
                                class="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3 px-4 rounded-lg shadow-md transition">
                                保存して更新
                            </button>
                            @if($billing->exists)
                                <a href="{{ route('billings.pdf', $billing) }}" target="_blank"
                                    class="block w-full bg-gray-700 hover:bg-gray-800 text-white font-bold py-3 px-4 rounded-lg shadow-md text-center transition">
                                    請求書PDF発行
                                </a>
                            @endif
                        </div>
